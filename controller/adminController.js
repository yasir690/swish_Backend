...
export const getAllPayment = async (req, res) => {
  try {
    ...
    // Step 2: Retrieve and process the payments based on the groupBy query parameter
    const { groupBy } = req.query; // Get groupBy parameter from the request

    // Ensure we default to 'monthly' if no groupBy is provided
    const groupByFormat = groupBy || "monthly";

    // Step 3: Define query to aggregate data
    const query = {
      $match: {
    ...
    // Handle the groupings based on the groupBy parameter
    if (groupByFormat === "monthly") {
      // Aggregate payments by month and week of the month (4 weeks)
      payments = await purchaseModel.aggregate([
        query,
        {
          $group: {
            _id: {
              $dateToString: {
                format: "%Y-%m-%U", // Year-Month-Week number
      ...
      // Prepare weekly data for the selected month (January example)
      ...
      payments.forEach((payment) => {
        const [year, month, week] = payment._id.split("-");
        const weekIndex = parseInt(week, 10) % 4; // Divide into 4 weeks

        if (month === (currentDate.getMonth() + 1).toString()) {
          weeklyData[weekIndex].y += payment.totalAmount;
        }
      });

      return res.status(200).json({
        success: true,
        message: "Payments grouped by week successfully.",
        data: weeklyData,
      });
    }

    if (groupByFormat === "yearly") {
      // Aggregate payments by year and month for the current year
      payments = await purchaseModel.aggregate([
        query,
        {
          $group: {
      ...
      // Initialize a structure for each month of the current year
      const yearlyData = [
        "Jan",
        "Feb",
        "Mar",
        ...
        "Nov",
        "Dec",
      ].map((month) => ({ x: month, y: 0 }));

      payments.forEach((payment) => {
        const [year, month] = payment._id.split("-");
        const monthIndex = parseInt(month, 10) - 1;
        if (year === currentDate.getFullYear().toString()) {
          yearlyData[monthIndex].y += payment.totalAmount;
        }
      });

      return res.status(200).json({
        success: true,
        message: "Payments grouped by month successfully.",
        data: yearlyData,
      });
    }

    if (groupByFormat === "weekly") {
      // Aggregate payments by each day of the week (Sun-Sat)
      ...
      payments = await purchaseModel.aggregate([
        query,
        {
          $group: {
            _id: {
              $dateToString: {
                format: "%Y-%U-%w", // Year-Week-Day of Week (0=Sunday, 6=Saturday)
                date: { $toDate: "$purchaseDate" },
              },
            },
            totalAmount: { $sum: "$price" },
      ...
      payments.forEach((payment) => {
        const [year, week, day] = payment._id.split("-");
        const dayIndex = parseInt(day, 10); // 0 for Sunday, 6 for Saturday
        if (
          new Date(payment._id).getFullYear() === currentDate.getFullYear() &&
          new Date(payment._id).getWeek() === currentDate.getWeek()
        ) {
          weeklyData[dayIndex].y += payment.totalAmount;
        }
      });

      return res.status(200).json({
        success: true,
        message: "Payments grouped by day of week successfully.",
        data: weeklyData,
      });
    }

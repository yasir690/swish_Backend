...
export const playShootingGame = async (req, res) => {
  try {
    ...
    const { Method, switch_type, spotSelection, penaltySpot } = req.body;
    ...
    if (userType === "child") {
      ...
      if (childpurchase) {
        ...
        console.log(childpurchase.subscriptionLevel, "test");
        ...
        if (childpurchase.subscriptionLevel === "FREE") {
          ...
          if (remainingTime <= 0 && spotSelection?.length > 0) {
            if (!penaltySpot || penaltySpot?.length == 0) {
              return res.status(400).json({
                success: false,
                message:
            ...
            req.body.spotSelection = [];
            req.response_message =
              "Your free subscription has expired. Buy a Basic subscription to use spot selection";
          }
